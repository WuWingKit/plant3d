"""
06_stage3_register_v4.py — 使用时间戳辅助配准

关键改进：
1. 使用时间戳计算每帧的理论旋转角度
2. 使用理论旋转角度作为 ICP 的初始变换
3. 只使用一圈的数据
"""
import os
import sys
...
def calculate_rotation_angles(timestamps, rotation_speed=18.157):
    """根据时间戳计算每帧的理论旋转角度"""
    angles = []
    cumulative_angle = 0
    for i in range(len(timestamps) - 1):
        ...
        angles.append({
            ...
            "interval": interval,
            "angle": angle,
            "cumulative": cumulative_angle
        })
    return angles


def create_rotation_matrix(angle_deg, axis='y'):
    """创建旋转矩阵"""
...
def pairwise_register_with_rotation(src, tgt, src_ds, tgt_ds, src_fpfh, tgt_fpfh,
                                     voxel, init_rotation, use_color_icp=True):
    """使用理论旋转角度作为初始变换的配准"""
    log = {}

    # 使用理论旋转角度作为初始变换
    init_T = init_rotation

    # 几何 ICP 精配
    geo_result, T_after_geo = icp_geometric(src, tgt, init_T, voxel)
    if geo_result is None:
        return init_T, np.eye(6), False, log
...
def main():
    parser = argparse.ArgumentParser(description="Stage 3: 使用时间戳辅助配准")
    ...
    # 计算理论旋转角度
    rotation_angles = calculate_rotation_angles(timestamps, args.rotation_speed)
    print(f"  计算 {len(rotation_angles)} 个旋转角度")

    # 找出转完一圈的位置
    one_turn_frame = None
    for angle_info in rotation_angles:
        if angle_info["cumulative"] >= 360:
            one_turn_frame = angle_info["id"]
            print(f"  转完一圈的位置: 帧 {one_turn_frame} (累积 {angle_info['cumulative']:.1f}°)")
            break

    if one_turn_frame is None:
        print("✗ 找不到转完一圈的位置")
        sys.exit(1)

    # 只使用一圈的数据
    pcd_files = sorted(glob.glob(os.path.join(pcds_dir, "*.ply")))
    pcd_files = pcd_files[:one_turn_frame]
    ...
    for i in range(len(pcds_data) - 1):
        ...
        # 计算理论旋转角度
        angle_info = rotation_angles[i]
        init_rotation = create_rotation_matrix(angle_info["angle"], axis='y')

        T, info, success, log = pairwise_register_with_rotation(
            src_data["pcd"], tgt_data["pcd"],
            src_data["ds"], tgt_data["ds"],
            src_data["fpfh"], tgt_data["fpfh"],
            args.voxel, init_rotation, not args.no_color_icp)
        ...
        if (i + 1) % 5 == 0 or i == len(pcds_data) - 2:
            print(f"    {i+1}/{len(pcds_data)-1}  "
                  f"理论旋转={angle_info['angle']:.2f}°  "
                  f"fit={log.get('color_fitness',0):.2f}  {status}")
    ...
    # 边日志
    log_path = os.path.join(out_dir, "registration_log_v4.csv")
