"""
06_stage3_register_v3.py — 只使用一圈数据的配准
...
def build_pose_graph(pcds_data, voxel, use_color_icp=True):
    """构建 pose graph"""
    n = len(pcds_data)
...
def main():
    parser = argparse.ArgumentParser(description="Stage 3: 只使用一圈数据的配准")
    ...
    parser.add_argument("--n-frames", type=int, default=None,
                        help="手动指定一圈的帧数（覆盖自动检测）")
    args = parser.parse_args()

    print("=" * 64)
    print("Stage 3: 只使用一圈数据的配准")
    print("=" * 64)
    ...
    if args.n_frames is not None:
        one_turn_frames = args.n_frames
        print(f"使用手动指定的一圈帧数: {one_turn_frames}")
    else:
        one_turn_frames = find_one_turn(pcd_files)
        print(f"自动检测一圈帧数: {one_turn_frames}")

    # 只使用一圈的数据
    pcd_files = pcd_files[:one_turn_frames]
    print(f"使用帧 0-{one_turn_frames-1} (共 {one_turn_frames} 帧)")
