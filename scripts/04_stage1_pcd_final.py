"""
04_stage1_pcd_final.py — 最终版本：直接用 Y 坐标去除地板
...
def make_colored_pcd_final(depth_path, aligned_path, K_depth, depth_trunc=None,
                           z_min=500.0, y_floor=0.0, auto_detect=True):
    """
    从 depth + aligned RGB 生成彩色点云

    参数:
        y_floor: 地板 Y 坐标阈值，低于此值的点会被去除
                 盆栽通常在 Y > 0 区域，地板在 Y < 0 区域
    """
    ...
    # 过滤条件：
    # 1. 深度范围：z_min ~ depth_trunc（去除近处杂点和远处背景墙壁）
    # 2. Y 坐标：> y_floor（去除地板）
    # 3. 空间位置：去除图像边缘 30 像素
...
def main():
    parser = argparse.ArgumentParser(description="Stage 1: 最终版本")
    ...
    parser.add_argument("--y-floor", type=float, default=0.0,
                        help="地板 Y 坐标阈值 mm（默认 0，低于此值去除）")
    ...
    for k, fid in enumerate(sel_ids):
        ...
        result, err = make_colored_pcd_final(
            d_path, a_path, K_d,
