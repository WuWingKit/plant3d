"""
06_stage3_register_theory.py — 直接使用理论旋转角度

不使用 ICP 配准，直接使用时间戳计算的理论旋转角度
"""
import os
import sys
import io
...
def find_one_turn(pcd_files, timestamps, search_range=(100, 150)):
    """找到转完一圈的位置"""
    print(f"\n  找到转完一圈的位置...")
...
def main():
    parser = argparse.ArgumentParser(description="Stage 3: 直接使用理论旋转角度")
    ...
    one_turn_frame = find_one_turn(pcd_files, timestamps, (args.search_start, args.search_end))
    ...
    for k, idx in enumerate(sample_indices):
        ...
        poses.append({"id": fid, "pose": R.tolist()})
